...
from glance_store._drivers.filesystem import FSBackendFile
...
class TestFSBackendObject(base.StoreBaseTest):
    """Test the FSBackendFile class."""

    def setUp(self):
        """Establish a clean test environment."""
        super(TestFSBackendObject, self).setUp()
        self.orig_chunksize = FSBackendFile.READ_CHUNKSIZE
        FSBackendFile.READ_CHUNKSIZE = 10
        self.store = FSBackendFile(datadir=self.test_dir)

    def tearDown(self):
        """Clear the test environment."""
        super(TestFSBackendObject, self).tearDown()
        ChunkedFile.CHUNKSIZE = self.orig_chunksize
    ...
    def _store_image(self, in_metadata):
        ...
        image_file = StringIO.StringIO(expected_file_contents)
        self.store.FILESYSTEM_STORE_METADATA = in_metadata
        return self.store.write(expected_image_id, image_file,
                              expected_file_size)
    ...
    def test_read(self):
        ...
        loc, size, checksum, = self.store.write(image_id,
                                                image_file,
                                                len(file_contents))
        ...
        uri = "file:///%s/%s" % (self.test_dir, image_id)
        loc= location.get_location_from_uri(uri, conf=self.conf)
        image_data= self.store.read(loc)

        expected_data = "chunk00000remainder"

        self.assertEqual(expected_data, image_data)
...
#        self.assertEqual(image_size, chunk_size)
    ...
    def test_write(self):
        ...
        loc, size, checksum = self.store.write(expected_image_id,
                                                image_file,
                                                expected_file_size)
        ...
        self.assertEqual(expected_file_contents, image_data)
...
#        self.assertEqual(metadata, {})
    ...
    def test_write_already_existing(self):
        """
        Tests that adding an image with an existing identifier
        raises an appropriate exception
        """
        ...
        location, size, checksum = self.store.write(image_id,
                                                     image_file,
                                                     size=file_size)
        image_file = StringIO.StringIO("nevergonnamakeit")
        self.assertRaises(exceptions.Duplicate,
    ...
    def _do_test_write_failure(self, errno, exception):
        ChunkedFile.CHUNKSIZE = units.Ki
        image_id = str(uuid.uuid4())
        file_size = 5 * units.Ki  # 5K
        file_contents = "*" * file_size
    ...
    def test_write_storage_full(self):
        """
        Tests that adding an image without enough space on disk
        raises an appropriate exception
        """
        self._do_test_write_failure(errno.ENOSPC, exceptions.StorageFull)

    def test_write_file_too_big(self):
        """
        Tests that adding an excessively large image file
        raises an appropriate exception
        """
        self._do_test_write_failure(errno.EFBIG, exceptions.StorageFull)

    def test_write_storage_write_denied(self):
        """
        Tests that adding an image with insufficient filestore permissions
        raises an appropriate exception
        """
        self._do_test_write_failure(errno.EACCES,
                                        exceptions.StorageWriteDenied)

    def test_write_other_failure(self):
        """
        Tests that a non-space-related IOError does not raise a
        StorageFull exceptions.
        """
        self._do_test_write_failure(errno.ENOTDIR, IOError)

    def test_write_cleanup_on_read_failure(self):
        """
        Tests the partial image file is cleaned up after a read
        failure.
        """
    ...
    def test_delete(self):
        ...
        self.assertRaises(exceptions.NotFound, self.store.read, loc)
    ...
    def test_delete_non_existing(self):
        ...
        raises an error
        """
        loc = "file:///tmp/glance-tests/non-existing"
        self.assertRaises(exceptions.NotFound,
                          self.store.delete,
                          loc)
